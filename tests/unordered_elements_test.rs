...
fn unordered_elements_are_matches_empty_array() -> Result<()> {
    let value = json!([]);
    verify_that!(value, j::unordered_elements_are![])
}
...
fn unordered_elements_are_matches_array() -> Result<()> {
    ...
    verify_that!(
        value,
        j::unordered_elements_are![eq("a"), starts_with("g"), eq("c")]
    )
}
...
fn unordered_elements_are_matches_unordered() -> Result<()> {
    let value = json!(["a", "b"]);
    verify_that!(value, j::unordered_elements_are![eq("b"), eq("a")])
}
...
fn unordered_elements_are_matches_unordered_with_repetition() -> Result<()> {
    ...
    verify_that!(
        value,
        j::unordered_elements_are![eq("a"), eq("a"), eq("a"), eq("b"), eq("b")]
    )
}
...
fn unordered_elements_are_matches_with_trailing_comma_in_list() -> Result<()> {
    ...
    verify_that!(
        value,
        j::unordered_elements_are![eq("a"), eq("b"), eq("c"),]
    )
}

#[test]
fn unordered_elements_are_explains_mismatch_due_to_wrong_size() -> Result<()> {
    let matcher = j::unordered_elements_are![eq("b"), eq("c"), eq("d")];
    verify_that!(
...
fn unordered_elements_are_description_no_full_match() -> Result<()> {
    let matcher = j::unordered_elements_are![eq("a"), eq("b"), eq("b")];
    verify_that!(
...
fn unordered_elements_are_unmatchable_expected_description_mismatch() -> Result<()> {
    let matcher = j::unordered_elements_are![eq(1), eq(2), eq(3)];
    verify_that!(
...
fn unordered_elements_are_unmatchable_actual_description_mismatch() -> Result<()> {
    let matcher = j::unordered_elements_are![eq("a"), eq("a"), eq("c")];
    verify_that!(
...
fn unordered_elements_are_fails_with_extra_actual_elements() -> Result<()> {
    ...
    verify_that!(
        value,
        not(j::unordered_elements_are![eq("a"), eq("b"), eq("c")])
    )
}
...
fn unordered_elements_are_matches_nested_unordered_arrays() -> Result<()> {
    ...
    verify_that!(
        ...
        j::unordered_elements_are![
            j::unordered_elements_are![eq("z")],
            j::unordered_elements_are![eq("y"), eq("x")]
        ]
    )
}
#[test]
...
fn unordered_elements_are_all_mismatch_unmatchable_message() -> Result<()> {
    ...
    let result = verify_that!(
        value,
        j::unordered_elements_are![eq("only-x"), eq("only-y")]
    );
    verify_that!(
        result,
...
fn unordered_elements_are_nested_arrays_match() -> Result<()> {
    ...
    verify_that!(
        value,
        j::unordered_elements_are![
            j::unordered_elements_are![eq(1), eq(2)],
            j::unordered_elements_are![eq(3), eq(4)]
        ]
    )
}
...
fn unordered_elements_are_mixed_types_match_with_owned_values() -> Result<()> {
    ...
    let one = json!(1);
    let t = json!(true);
    verify_that!(value, j::unordered_elements_are![a, one, t])
}
...
fn unordered_elements_are_mixed_types_match_with_inline_borrowed_literals() -> Result<()> {
    verify_that!(
        json!(["a", true, 1]),
        j::unordered_elements_are![&json!("a"), &json!(1), &json!(true)]
    )
}
...
fn unordered_elements_are_mixed_types_match_with_mixed_owned_and_borrowed() -> Result<()> {
    let value = json!([1, true, "a"]);
    let a = json!("a");
    verify_that!(value, j::unordered_elements_are![a, json!(1), &json!(true)])
}
...
fn unordered_elements_are_mixed_types_match_with_owned_values_unmatch() -> Result<()> {
    ...
    let one = json!(1);
    let t = json!(true);
    verify_that!(value, not(j::unordered_elements_are![a, one, t]))
}
...
fn unordered_elements_are_mixed_types_match_with_mixed_owned_and_borrowed_unmatch() -> Result<()> {
    ...
    verify_that!(
        value,
        not(j::unordered_elements_are![a, json!(1), &json!(true)])
    )
}
...
fn unordered_elements_are_matches_with_primitive_literals() -> Result<()> {
    ...
    verify_that!(value, j::unordered_elements_are!["a", 1i64, true])
}

#[test]
fn unordered_elements_are_unmatch_with_primitive_literals() -> Result<()> {
    let value = json!(["a", 1, false]);
    verify_that!(value, not(j::unordered_elements_are!["a", 2i64, true]))
}
...
fn unordered_elements_are_matches_with_mixed_literals_and_matchers() -> Result<()> {
    ...
    verify_that!(
        json!(["alex", 1, true]),
        j::unordered_elements_are![starts_with("a"), a, is_true()]
    )
}

#[test]
fn unordered_elements_describe_no_match_when_negated() -> Result<()> {
    let result = verify_that!(json!([1, 2]), not(j::unordered_elements_are![1, 2]));
    verify_that!(
        result,
        err(displays_as(contains_substring(
