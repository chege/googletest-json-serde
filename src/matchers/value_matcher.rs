...
pub mod internal {
    ...
    /// `JsonValueMatcher` is an adapter matcher that extracts a value (string, number, or bool)
    /// from a JSON `Value` and delegates to an inner matcher operating on that value type.
    #[doc(hidden)]
    #[derive(MatcherBase)]
    ...
    impl<M, T> JsonValueMatcher<M, T> {
        /// Constructs a new `JsonValueMatcher` adapter with the provided inner matcher.
